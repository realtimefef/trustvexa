...
const BADGES = [
  { icon: BadgeCheck, label: 'Identity verified', hint: 'Email and account checks passed.' },
  {
    ...
    hint: 'Trades secured through TrustVexa escrow.',
  },
...
const REVIEWS = [
  {
    author: 'crypto_mara',
    rating: 5,
    body: 'Smooth release and clear communication the whole way through. Would trade again.',
    deal: 'USDT · ERC-20',
  },
  {
    author: 'ledger_l24',
    rating: 5,
    body: 'Funded fast and followed the agreed terms exactly. No surprises at settlement.',
    deal: 'BTC · Lightning',
  },
  {
    author: 'nodewrangler',
    rating: 4,
    body: 'Great counterparty. Delivery took a little longer but everything checked out.',
    deal: 'ETH · Mainnet',
  },
];
...
const RELATED_LINKS = [
  ...
  { href: '/settings#danger-zone', label: 'Delete account', hint: 'Permanently close your profile.' },
];
...
export default function ProfilePage() {
  ...
  // Sync state with queried profile
  React.useEffect(() => {
    if (profile) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      <DashboardPageHeader
        title="Profile"
        description="Your public trading identity and reputation."
      />
      ...
      {isLoading ? (
        ...
        <Card className="overflow-hidden rounded-2xl shadow-soft">
          <div className="relative h-32 bg-brand-gradient">
            <div
              aria-hidden="true"
              className="absolute inset-0 bg-grid opacity-20 [mask-image:radial-gradient(ellipse_at_center,black,transparent_75%)]"
            />
          </div>
            ...
            <div className="-mt-12 flex flex-wrap items-end justify-between gap-4">
              <div className="flex items-end gap-4">
                <div className="relative h-24 w-24 rounded-2xl border-4 border-card overflow-hidden bg-brand-gradient flex items-center justify-center shadow-glow">
                  {profile?.avatarUrl ? (
                    <img
                      src={`${API_ORIGIN}${profile.avatarUrl}?t=${avatarTimestamp}`}
                      alt={username}
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        <StatCard
          icon={Star}
          label="Reputation"
          value={`${(4.0 + trustLevel * 0.2).toFixed(1)} / 5`}
          hint="Trust rating"
          accent="warning"
        />
      ...
      <Card className="rounded-2xl shadow-soft">
        ...
        <CardContent>
          <form className="grid gap-5 sm:grid-cols-2" onSubmit={handleSubmit}>
            {saveError && (
              <div className="p-3 text-xs bg-destructive/10 text-destructive rounded-lg border border-destructive/20 sm:col-span-2">
                {saveError}
              </div>
            )}
            {saveSuccess && (
              <div className="p-3 text-xs bg-success/15 text-success rounded-lg border border-success/20 sm:col-span-2">
      ...
      <Reveal className="space-y-6">
        <SectionHeading
          ...
          title="How your trust level grows"
          subtitle="Trust on TrustVexa is earned, not bought. Every clean, dispute-free settlement raises your standing with future counterparties."
        />
        <div className="grid gap-5 sm:grid-cols-3">
        ...
        <Card className="rounded-2xl border bg-muted/30 shadow-soft">
          <CardContent className="flex flex-wrap items-center gap-4 p-6">
            ...
            <p className="flex-1 text-sm text-muted-foreground">
              Your reputation score blends completed deal volume, counterparty ratings, and dispute
              history. Scores recover over time, so a single rough trade never defines you.
            </p>
          </CardContent>
        </Card>
      </Reveal>
      ...
      <Reveal className="space-y-6">
        <SectionHeading
          ...
          title="Your verification badges"
          subtitle="Badges are visual shorthand for the checks you've passed. Counterparties see these the moment they open a deal with you."
        />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {BADGES.map((badge) => (
            <div
              key={badge.label}
              className="flex flex-col gap-3 rounded-2xl border bg-card p-5 shadow-soft transition-all hover:-translate-y-1 hover:shadow-glow"
            >
        ...
        <p className="flex items-center gap-2 text-xs text-muted-foreground">
          <Fingerprint className="h-4 w-4 text-primary" aria-hidden="true" /> Badges update
          automatically as you complete checks and deals — there's nothing to claim manually.
        </p>
      </Reveal>
        ...
        <Card className="rounded-2xl border bg-card shadow-soft">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 font-display">
              <Lock className="h-5 w-5 text-primary" aria-hidden="true" /> Privacy: what others see
            </CardTitle>
            <CardDescription>
              We show counterparties only what they need to trade with confidence — nothing more.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border bg-muted/30 p-4">
              <p className="flex items-center gap-2 text-sm font-medium text-success">
                <Eye className="h-4 w-4" aria-hidden="true" /> Visible to others
              </p>
            ...
            <div className="rounded-xl border bg-muted/30 p-4">
              <p className="flex items-center gap-2 text-sm font-medium text-muted-foreground">
                <EyeOff className="h-4 w-4" aria-hidden="true" /> Never shared
              </p>
              <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
      ...
      <Reveal className="space-y-6">
        <SectionHeading
          ...
          subtitle="A sample of how counterparty feedback appears on your profile. These illustrative entries show the format buyers and sellers leave after a deal settles."
        />
        <div className="grid gap-5 lg:grid-cols-3">
          {REVIEWS.map((review) => (
            <Card key={review.author} className="rounded-2xl border bg-card shadow-soft">
              <CardHeader>
                <div className="flex items-center justify-between">
        ...
        <p className="flex items-center gap-2 text-xs text-muted-foreground">
          <ThumbsUp className="h-4 w-4 text-success" aria-hidden="true" /> Reviews can only be left
          by a verified counterparty after a deal reaches settlement.
        </p>
      </Reveal>
      ...
      <Reveal>
        <Card className="rounded-2xl border bg-muted/30 shadow-soft">
          <CardHeader>
            ...
            <CardDescription>
              Jump to the tools that keep your account in good standing.
            </CardDescription>
          </CardHeader>
