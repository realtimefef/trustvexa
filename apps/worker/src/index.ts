...
import { startHealthServer, type HeartbeatLiveness } from './health.js';
import { db, withTransaction } from './db.js';
import { createDefaultAdapters } from './adapters.js';
import {
  PROCESSOR_REGISTRY,
  type JobProcessor,
  type ProcessableQueue,
...
const SLA_TIMERS_QUEUE = 'sla-timers' as const;
const LIFECYCLE_SWEEP_JOB = 'lifecycle-sweep' as const;
 ...
 */
const DEPOSIT_WATCH_INTERVAL_MS = Number.parseInt(
  process.env.DEPOSIT_WATCH_INTERVAL_MS ?? '30000',
  10,
);
const DEPOSIT_WATCH_QUEUE = 'deposit-watch' as const;
const DEPOSIT_WATCH_JOB = 'deposit-sweep' as const;
 ...
 */
let processorContext: ProcessorContext | null = null;
function getProcessorContext(): ProcessorContext {
 ...
 */
const heartbeatState: { lastBeatAtMs: number | null } = { lastBeatAtMs: null };

/** Snapshot the current heartbeat liveness for the health endpoint. */
function getHeartbeatLiveness(): HeartbeatLiveness {
...
async function placeholderProcessor(job: Job): Promise<void> {
  logger.info(
    { queue: job.queueName, job_id: job.id, job_name: job.name },
    'received job (scaffold no-op)',
  );
}
 ...
 */
async function slaTimerProcessor(job: Job): Promise<void> {
  ...
  logger.info(
    { queue: job.queueName, job_id: job.id, ...summary },
    'lifecycle timer sweep complete',
  );
}
 ...
 */
function processorFor(name: QueueName): (job: Job) => Promise<void> {
  if (name === SLA_TIMERS_QUEUE) return slaTimerProcessor;
  if (name === DEPOSIT_WATCH_QUEUE) {
    return (job: Job) => processDepositWatch(job, getProcessorContext());
  }
  ...
  const handler = (PROCESSOR_REGISTRY as Record<string, JobProcessor | undefined>)[name];
  if (handler) {
    return (job: Job) => handler(job, getProcessorContext());
  }
  return placeholderProcessor;
}

/** Compile-time guard: every ProcessableQueue is also a registered QueueName. */
const _registryQueuesAreKnown: ReadonlyArray<QueueName> = Object.keys(
  PROCESSOR_REGISTRY,
) as ProcessableQueue[];
 ...
 */
function bootstrapQueues(): { queues: Map<QueueName, Queue>; workers: Worker[] } {
  ...
  for (const name of QUEUE_NAMES) {
    ...
    worker.on('failed', (job: Job | undefined, err: Error) => {
      ...
      if (job) {
        void recordJobFailure(getProcessorContext(), name, job, err).catch((dlErr: unknown) => {
          logger.error(
            { queue: name, job_id: job.id ?? 'unknown', err: dlErr instanceof Error ? dlErr.message : String(dlErr) },
 ...
 */
async function scheduleLifecycleSweep(queues: Map<QueueName, Queue>): Promise<void> {
 ...
 */
async function scheduleDepositWatchSweep(queues: Map<QueueName, Queue>): Promise<void> {
 ...
 */
async function scheduleBackupRestoreTest(queues: Map<QueueName, Queue>): Promise<void> {
 ...
 */
async function scheduleDbBackup(queues: Map<QueueName, Queue>): Promise<void> {
 ...
 * (Audit FIX-P2-3)
 */
async function scheduleIdempotencyCleanup(queues: Map<QueueName, Queue>): Promise<void> {
 ...
 */
async function scheduleEmailDigest(queues: Map<QueueName, Queue>): Promise<void> {
...
export function main(): WorkerRuntime {
  assertRuntimeConfig();
  ...
  const runtime: WorkerRuntime = { queues, workers, heartbeat, healthServer };

  void scheduleLifecycleSweep(queues).catch((err: unknown) => {
    logger.error(
      { err: err instanceof Error ? err.message : String(err) },
      'failed to schedule lifecycle sweep',
    );
  });

  void scheduleDepositWatchSweep(queues).catch((err: unknown) => {
    logger.error(
      { err: err instanceof Error ? err.message : String(err) },
      'failed to schedule deposit-watch sweep',
    );
  });

  void scheduleBackupRestoreTest(queues).catch((err: unknown) => {
    logger.error(
      { err: err instanceof Error ? err.message : String(err) },
      'failed to schedule backup-restore test sweep',
    );
  });

  void scheduleDbBackup(queues).catch((err: unknown) => {
    logger.error(
      { err: err instanceof Error ? err.message : String(err) },
      'failed to schedule daily DB backup',
    );
  });

  void scheduleEmailDigest(queues).catch((err: unknown) => {
    logger.error(
      { err: err instanceof Error ? err.message : String(err) },
      'failed to schedule email digest cron',
    );
  });

  void scheduleIdempotencyCleanup(queues).catch((err: unknown) => {
    logger.error(
      { err: err instanceof Error ? err.message : String(err) },
      'failed to schedule idempotency-keys cleanup',
    );
  });
  ...
  logger.info(
    { queues: queues.size, heartbeat_interval_ms: HEARTBEAT_INTERVAL_MS },
    'worker bootstrapped',
  );
  return runtime;
}
