...
describe('processPayoutProcessing', () => {
  it('handles broadcast action successfully', async () => {
    ...
    const mockContext = {
      ...
      db: mockDb,
      withTransaction: async (fn: any) => fn(mockDb),
      adapters: {
        chain: {
  ...
  it('handles broadcast version mismatch', async () => {
    ...
    expect(mockContext.logger.warn).toHaveBeenCalledWith(
      { payout_id: 'payout-1', expected_version: 2 },
      'payout broadcast: optimistic version conflict, skipping',
    );
  });
