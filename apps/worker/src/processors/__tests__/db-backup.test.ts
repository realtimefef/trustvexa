...
vi.mock('@trustvexa/api/worker-jobs', async (importOriginal) => {
  const original = await importOriginal<typeof import('@trustvexa/api/worker-jobs')>();
  const mockStorage = {
    put: vi.fn(),
  };
  return {
...
describe('processDbBackup', () => {
  ...
  it('skips backup if DATABASE_URL is not set', async () => {
    ...
    await processDbBackup({} as unknown as Job, mockContext);

    expect(mockContext.logger.error).toHaveBeenCalledWith(
      'DATABASE_URL not set. Skipping scheduled DB backup.',
    );
  });
  ...
  it('executes backup workflow successfully', async () => {
    ...
    expect(execFileSync).toHaveBeenCalledWith(
      'pg_dump',
      ['postgres://localhost/test', '-f', expect.any(String), '--no-owner', '--clean'],
      { stdio: ['ignore', 'ignore', 'pipe'] },
    );

    expect(fs.readFileSync).toHaveBeenCalled();
    expect(storage.put).toHaveBeenCalledWith(
      expect.stringContaining('backups/db_backup_'),
      mockBytes,
      'application/sql',
    );
  ...
  it('handles backup failure and enqueues alert email', async () => {
    ...
    vi.mocked(Queue).mockImplementation(() => queueInstanceMock as any);

    await expect(processDbBackup({} as unknown as Job, mockContext)).rejects.toThrow(
      'pg_dump failed',
    );
