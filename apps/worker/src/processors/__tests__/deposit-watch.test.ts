...
describe('processDepositWatch', () => {
  ...
  it('runs empty sweep when no pending addresses await funding', async () => {
    ...
    await processDepositWatch({ data: {} } as any, mockContext);

    expect(escrowAddressRepo.listEscrowAddressesAwaitingFunding).toHaveBeenCalledWith(
      mockContext.db,
    );
  ...
  it('processes incoming transfers and credits them if they meet threshold', async () => {
    ...
    vi.mocked(loadChainClient).mockResolvedValue(mockChainClient as any);
    ...
    expect(depositRepo.upsertPayment).toHaveBeenCalledWith(
      expect.any(Object),
      expect.objectContaining({
        dealId: 'deal-1',
        coin: 'USDT',
