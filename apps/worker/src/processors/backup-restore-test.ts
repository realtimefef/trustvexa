...
export async function runBackupRestoreTest(ctx: ProcessorContext): Promise<void> {
  ...
  try {
    ...
    await ctx.db.query(
      `INSERT INTO backup_jobs (backup_type, status, completed_at) VALUES ('restore_test', 'success', now())`,
    );
    ...
    await ctx.db.query(
      `INSERT INTO backup_jobs (backup_type, status, completed_at) VALUES ('restore_test', 'failed', now())`,
    );
    throw err;
  } finally {
    // 5. Clean up
