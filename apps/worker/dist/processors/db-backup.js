...
export async function processDbBackup(_job, ctx) {
    ...
    const insertRes = await ctx.db.query(`INSERT INTO backup_jobs (backup_type, status, started_at)
    ...
    try {
        ...
        await ctx.db.query(`UPDATE backup_jobs 
       SET status = 'success', completed_at = $1, storage_location = $2
       WHERE id = $3`, [completedAt, storageKey, dbBackupJobId]);
        ctx.logger.info({ storageKey }, 'Database backup complete and stored in object storage.');
        // Clean up local temp file
        fs.unlinkSync(dumpPath);
    }
    catch (err) {
        ctx.logger.error({ err: err instanceof Error ? err.message : String(err) }, 'Database backup failed');
        const completedAt = new Date();
        if (dbBackupJobId) {
            await ctx.db.query(`UPDATE backup_jobs 
         SET status = 'failed', completed_at = $1
         WHERE id = $2`, [completedAt, dbBackupJobId]);
        }
