...
import { getDigestEmail } from '../email-templates/index.js';
 ...
 */
function frequencyFromJobName(name) {
...
export async function processEmailDigest(job, ctx) {
    ...
    const usersResult = await ctx.db.query(`SELECT u.id AS user_id, u.email_enc, u.username
       FROM users u
       JOIN user_preferences up ON up.user_id = u.id
      WHERE up.email_digest_frequency = $1`, [frequency]);
    if (usersResult.rows.length === 0) {
        ctx.logger.info({ frequency }, 'no users with this digest frequency — skipping');
        return;
    }
    ctx.logger.info({ frequency, user_count: usersResult.rows.length }, 'processing digest for users');
    let emailsQueued = 0;
    ...
    try {
        for (const user of usersResult.rows) {
            ...
            const notifsResult = await ctx.db.query(`SELECT id, type, payload::text, deal_id, created_at
           FROM notifications
          WHERE user_id = $1
            AND read_at IS NULL
          ...
          LIMIT 50`, [user.user_id, cutoff]);
            ...
            const digestItems = notifsResult.rows.map((row) => {
                ...
                return {
                    type: row.type,
                    dealTitle: payload.dealTitle || undefined,
            ...
            await ctx.db.query(`UPDATE notifications
            SET digest_sent_at = now()
          WHERE id = ANY($1)`, [notifIds]);
            emailsQueued++;
        }
    }
    finally {
    ...
    ctx.logger.info({ frequency, users_checked: usersResult.rows.length, emails_queued: emailsQueued }, 'email digest sweep complete');
}
//# sourceMappingURL=email-digest.js.map