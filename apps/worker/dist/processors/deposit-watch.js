...
import { confirmations as confirm, depositClassification as classify, depositRepo, escrowAddressRepo, escrowAddressView, openPii, precision, payoutRepo, } from '@trustvexa/api/worker-jobs';
import { parseDepositWatchJob } from '../payloads.js';
...
function toConfirmationState(network, result) {
    if (result === 'finalized') {
...
const CREDITED_STATUSES = new Set(['credited', 'confirmed']);
export async function processDepositWatch(job, ctx) {
    const data = parseDepositWatchJob(job.data);
    const filter = data.networks === undefined ? null : new Set(data.networks);
    ...
    for (const row of pending) {
        if (!isSupportedNetwork(row.network))
            continue;
        if (filter !== null && !filter.has(row.network))
            continue;
        if (clients.has(row.network))
            continue;
        const client = await loadChainClient(row.network, process.env);
        clients.set(row.network, client);
        if (client === null) {
            summary.chainsSkipped += 1;
            ctx.logger.warn({ network: row.network }, 'deposit-watch: chain RPC not configured/available — skipping (manual verification fallback)');
        }
        else {
    ...
    for (const row of pending) {
        ...
        if (client === null)
            continue; // skipped chain
        if (row.amount_smallest_unit === null)
            continue; // no funding snapshot yet
...
async function enqueueNotification(userId, dealId, eventType, payload) {
    const connection = createRedisConnection();
 ...
 * threshold (i.e. the deal is now funded by this deposit).
 */
async function creditTransfer(ctx, args) {
    ...
    const state = toConfirmationState(network, transfer.finalized === true ? 'finalized' : transfer.confirmations);
    ...
    const existingPayment = await ctx.db.query(`SELECT status FROM payments WHERE tx_hash = $1 AND output_index = $2 LIMIT 1`, [transfer.txHash, transfer.outputIndex]);
    const isNewOrChanged = existingPayment.rows.length === 0 || existingPayment.rows[0]?.status !== status;
    await ctx.withTransaction((tx) => depositRepo.upsertPayment(tx, {
        dealId,
        coin: transfer.coin,
        network,
    ...
    if (isNewOrChanged) {
        try {
            const connection = createRedisConnection();
            await connection.publish('realtime:deal:events', JSON.stringify({
                dealId,
        ...
        if (status === 'credited') {
            ...
            try {
                ...
                ctx.logger.error({ err: err instanceof Error ? err.message : String(err), dealId }, 'deposit-watch: failed to transition deal to Funded');
            }
            try {
                const dealResult = await ctx.db.query(`SELECT buyer_id FROM deals WHERE id = $1 LIMIT 1`, [dealId]);
                const deal = dealResult.rows[0];
                if (deal && deal.buyer_id) {
                    const connection = createRedisConnection();
                    await connection.publish('realtime:wallet:events', JSON.stringify({
        ...
        if (status === 'underpaid') {
            try {
                const dealResult = await ctx.db.query(`SELECT buyer_id, title FROM deals WHERE id = $1 LIMIT 1`, [dealId]);
                const deal = dealResult.rows[0];
                if (deal && deal.buyer_id) {
                    const buyerResult = await ctx.db.query(`SELECT username, email_enc FROM users WHERE id = $1 LIMIT 1`, [deal.buyer_id]);
                    const buyer = buyerResult.rows[0];
                    if (buyer) {
                        const rule = precision.getPrecisionRule(transfer.coin, network);
                        const expectedAmountCoin = precision.fromSmallestUnit(expected.amountSmallestUnit, rule.decimals);
                        const receivedAmountCoin = precision.fromSmallestUnit(transfer.amountSmallestUnit, rule.decimals);
                        const shortfallCoin = precision.fromSmallestUnit(classification.shortfallSmallestUnit, rule.decimals);
                        let buyerEmail = '';
                        if (buyer.email_enc) {
            ...
            `;
                ...
                ctx.logger.error({ err: err instanceof Error ? err.message : String(err) }, 'Failed to process underpayment notification');
            }
        }
            ...
            try {
                ...
                if (deal && deal.buyer_id) {
                    const rule = precision.getPrecisionRule(transfer.coin, network);
                    const excessAmountCoin = precision.fromSmallestUnit(classification.refundExcessSmallestUnit, rule.decimals);
                    const addressResult = await ctx.db.query(`SELECT address_enc FROM user_payout_addresses WHERE user_id = $1 AND coin = $2 AND network = $3 LIMIT 1`, [deal.buyer_id, transfer.coin, network]);
                    ...
                    if (!buyerRefundAddress) {
                        ctx.logger.error({ dealId, buyerId: deal.buyer_id, excessAmountCoin }, 'deposit-watch: overpayment refund skipped — buyer has no configured payout address. Manual admin intervention required.');
                    }
                    else {
                ...
                ctx.logger.error({ err: err instanceof Error ? err.message : String(err) }, 'Failed to process overpayment refund');
            }
        }
    }
    return fundedNow;
 ...
 */
async function detectReorgs(ctx, client, network, tier, dealId) {
    ...
    for (const payment of existing) {
        ...
        if (confirm.meetsThreshold(state, tier))
            continue; // still funded — no reorg
        await ctx.withTransaction(async (tx) => {
        ...
        reorgs += 1;
        ctx.logger.warn({ network, deal_id: dealId, tx_hash: payment.tx_hash }, 'deposit-watch: reorg detected — credited deposit dropped below threshold');
    }
    return reorgs;
}
//# sourceMappingURL=deposit-watch.js.map