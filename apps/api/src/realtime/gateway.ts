...
} from '../modules/chat/handshake.js';
...
import { checkRateLimit, type WindowState } from '../modules/chat/rate-limit.js';

/** Per-socket session state stored on the socket after a successful handshake. */
...
export interface GatewayDeps {
  ...
  loadDealAuth: (dealId: string) => Promise<{ buyerId: string; sellerId: string; middlemanId: string | null } | null>;
...
export function createGateway(httpServer: HttpServer, deps: GatewayDeps): Server {
  ...
  io.use(async (socket, next) => {
    ...
    (socket.data as SocketSession).identity = result.identity;
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('deal:join', async (payload: unknown, ack?: (ok: boolean) => void) => {
    ...
    socket.on('deal:leave', async (payload: unknown, ack?: (ok: boolean) => void) => {
      if (!payload || typeof payload !== 'object' || !('dealId' in payload)) {
        ack?.(false);
        return;
      }
    ...
    socket.on(CLIENT_EVENTS.joinChat, async (chatId: string, ack?: (ok: boolean) => void) => {
    ...
    socket.on(
      CLIENT_EVENTS.sendMessage,
      async (chatId: string, payload: unknown, ack?: (ok: boolean) => void) => {
        ...
        if (chatAuth) {
          const opponentIds = [chatAuth.parties.buyerId, chatAuth.parties.sellerId, chatAuth.parties.middlemanId]
            .filter((id): id is string => typeof id === 'string' && id !== session.identity.userId);
    ...
    const emitTyping = async (chatId: string, typing: boolean): Promise<void> => {
      if (!socket.rooms.has(chatRoom(chatId))) return;
      const ctx = await deps.loadChatAuth(chatId);
      if (!ctx) return;
      if (roleOf(session.identity.userId, ctx.parties) === 'middleman') return;
    ...
    socket.on(CLIENT_EVENTS.startTyping, (chatId: string) => {
      void emitTyping(chatId, true);
    });
    socket.on(CLIENT_EVENTS.stopTyping, (chatId: string) => {
    ...
    socket.on(CLIENT_EVENTS.markRead, async (chatId: string, messageId: string) => {
      if (!socket.rooms.has(chatRoom(chatId))) return;
      const ctx = await deps.loadChatAuth(chatId);
      if (!ctx) return;
      if (roleOf(session.identity.userId, ctx.parties) === 'middleman') return;
...
export function connectedUserRole(identity: HandshakeIdentity, parties: DealParties) {
