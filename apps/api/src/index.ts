...
export function main(): void {
  ...
  const server = app.listen(config.port, () => {
    logger.info(
      { port: config.port, prefix: API_PREFIX, env: config.nodeEnv },
      'TrustVexa API listening',
    );
  });
  ...
  try {
    ...
    const io = createGateway(server, {
      ...
      loadDealAuth: async (dealId) => {
        const { rows } = await query<{ buyer_id: string; seller_id: string; middleman_id: string | null }>(
