...
import {
  ...
  getPublicReviewById,
  insertPublicReview,
  listAllPublicReviews,
  listVisiblePublicReviews,
  setPublicReviewDeleted,
  setPublicReviewHidden,
  setPublicReviewReply,
  updatePublicReviewContent,
  type PublicReviewRow,
  type TxClient,
} from './public-reviews.repository.js';
...
function toPublicView(row: PublicReviewRow): PublicReviewView {
  return {
    id: row.id,
...
function toModerationView(row: PublicReviewRow): PublicReviewView {
  return {
    ...toPublicView(row),
...
export async function submitPublicReview(
  ...
  if (recent >= MAX_REVIEWS_PER_HOUR) {
    throw new AppError(
      'too_many_reviews',
      'You have posted too many reviews recently. Please try again later.',
      429,
    );
  }
...
export async function listPublicReviews(limit = 100, offset = 0): Promise<PublicReviewsResult> {
...
export async function listPublicReviewsForModeration(
  limit = 200,
  offset = 0,
...
async function withModerationTx<T>(
  ctx: ModerationContext,
  action: string,
  metadata: Record<string, unknown>,
  mutate: (tx: PooledTx, review: PublicReviewRow) => Promise<T>,
  ...
  try {
    ...
    const auditId = await appendAdminAction(client, {
      ...
      reason: (metadata.reason as string) ?? action,
...
export async function editPublicReview(
  input: EditPublicReviewInput,
): Promise<{ reviewId: string; auditId: string }> {
  const body = input.body.trim();
  if (body.length === 0) {
    throw new AppError('review_empty', 'Review text must not be empty.', 422);
  }
  const { auditId } = await withModerationTx(
    input,
    'edit_public_review',
    { reason: input.reason, rating: input.rating },
    async (tx) => {
...
export async function replyToPublicReview(
  input: ReplyPublicReviewInput,
): Promise<{ reviewId: string; auditId: string }> {
  const reply = input.reply?.trim() ? input.reply.trim() : null;
  const { auditId } = await withModerationTx(
    input,
    reply === null ? 'clear_public_review_reply' : 'reply_public_review',
    { reason: 'middleman reply' },
    async (tx) => {
...
export async function setPublicReviewVisibility(
  input: SetHiddenInput,
): Promise<{ reviewId: string; hidden: boolean; auditId: string }> {
  if (!input.reason?.trim()) {
    throw new AppError('reason_required', 'A reason is required to moderate a review.', 422);
  }
  const { auditId } = await withModerationTx(
    input,
    input.hidden ? 'hide_public_review' : 'unhide_public_review',
    { reason: input.reason },
    async (tx) => {
...
export async function deletePublicReview(
  input: DeletePublicReviewInput,
): Promise<{ reviewId: string; auditId: string }> {
  if (!input.reason?.trim()) {
