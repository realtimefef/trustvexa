...
export function documentsRouter(): Router {
  ...
  router.get(
    ...
    asyncHandler(controller.getDealDocuments),
  );

  // GET /api/v1/documents/deals/:id/agreement.pdf — the deal-agreement PDF.
  router.get(
    '/deals/:id/agreement.pdf',
    ...apiChain({ schemas: { params: dealIdParamSchema }, roles: ['user', 'middleman'] }),
    asyncHandler(controller.getAgreementPdf),
  );

  // GET /api/v1/documents/deals/:id/receipt.pdf — branded receipt for a settled deal.
  router.get(
    '/deals/:id/receipt.pdf',
    ...apiChain({ schemas: { params: dealIdParamSchema }, roles: ['user', 'middleman'] }),
    asyncHandler(controller.getReceiptPdf),
  );

  // GET /api/v1/documents/deals/:id/dispute-decision.pdf — final decision PDF.
  router.get(
    '/deals/:id/dispute-decision.pdf',
    ...apiChain({ schemas: { params: dealIdParamSchema }, roles: ['user', 'middleman'] }),
    asyncHandler(controller.getDisputeDecisionPdf),
  );
  ...
  router.get(
    ...
    asyncHandler(controller.getDataExport),
  );
