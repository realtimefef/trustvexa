...
export async function revokeJti(params: {
  ...
  await query(
    `INSERT INTO revoked_tokens (jwt_id, user_id, reason, revoked_at, expires_at)
     VALUES ($1, $2, $3, now(), $4)
     ON CONFLICT (jwt_id) DO NOTHING`,
    [params.jti, params.userId, params.reason, params.expiresAt.toISOString()],
  );
}
...
export async function isJtiRevoked(jti: string): Promise<boolean> {
  ...
  const res = await query<{ jwt_id: string }>(
    `SELECT jwt_id FROM revoked_tokens WHERE jwt_id = $1 LIMIT 1`,
    [jti],
  );
