...
async function revokeEverything(userId: string, reason: 'logout_all' | 'blocked'): Promise<void> {
...
async function requirePassword(userId: string, password: string): Promise<repo.UserRecord> {
...
export async function requestDeletion(
  ...
  if (flags?.legal_hold) {
    throw new AppError(
      'legal_hold',
      'This account is under a legal hold and cannot be deleted.',
      423,
    );
  }
