 ...
 */
async function maybeLockout(identifierHash: string): Promise<void> {
...
function toPublicUser(u: repo.UserRecord): PublicUser {
  return {
    id: u.id,
    username: u.username,
    role: u.account_type,
...
export async function register(input: RegisterInput, meta: RequestMeta): Promise<AuthResult> {
  ...
  const acceptances = await Promise.all(
    (['terms', 'privacy'] as const).map(async (docType) => ({
      docType,
...
export async function verifyTOTPAndLogin(
  input: { email: string; password: string; code: string; rememberMe?: boolean },
  meta: RequestMeta,
): Promise<AuthResult> {
  const cfg = getAuthConfig();
  const email = input.email.trim().toLowerCase();
...
export async function refresh(refreshToken: string, _meta: RequestMeta): Promise<AuthResult> {
  const cfg = getAuthConfig();
  ...
  try {
    ...
    const access = issueAccessToken(
      { userId: user.id, role: user.account_type, sessionId: claims.sid },
      cfg,
    );
 ...
 */
export async function verifySession(refreshToken: string): Promise<PublicUser> {
  const cfg = getAuthConfig();
...
export async function loginWithGoogle(
  ...
  meta: RequestMeta,
): Promise<AuthResult & { isNewAccount: boolean }> {
  const cfg = getAuthConfig();
