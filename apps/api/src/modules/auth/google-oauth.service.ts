...
async function exchangeCode(cfg: GoogleOAuthConfig, code: string): Promise<string> {
...
async function fetchProfile(accessToken: string): Promise<GoogleProfile> {
  ...
  return parseGoogleProfile((await res.json()) as Record<string, unknown>);
}

function usernameFromProfile(profile: GoogleProfile): string {
