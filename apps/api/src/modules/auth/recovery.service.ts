...
import { revokeJti, type RevocationReason } from './token-store.js';
import type {
  ChangeEmailInput,
  ChangePasswordInput,
  ForgotPasswordInput,
...
function emailHashOf(email: string, cfg: AuthConfig): string {
  return hashLookup(email.trim().toLowerCase(), cfg.lookupHashKey);
}

async function revokeAllSessionsForReason(userId: string, reason: RevocationReason): Promise<void> {
...
async function assertNotBreached(password: string, cfg: AuthConfig): Promise<void> {
  if (!cfg.breachCheckEnabled) return;
  try {
...
export async function forgotPassword(input: ForgotPasswordInput): Promise<void> {
  ...
  if (!user || user.account_status === 'deleted') {
    throw new AppError(
      'account_not_found',
      'No account is registered with this email address. Please check the address or create a new account.',
      404,
    );
  }
  if (user.account_status === 'blocked') {
    throw new AppError(
      'account_blocked',
      'This account has been suspended. Please contact support@trustvexa.com for assistance.',
      403,
    );
  }
...
export async function changeEmail(userId: string, input: ChangeEmailInput): Promise<void> {
  const cfg = getAuthConfig();
  const user = await repo.findUserById(userId);
  if (!user || !user.password_hash) {
    throw new AppError('not_found', 'Account not found.', 404);
...
export async function acceptPolicy(
  userId: string,
  input: { docType: 'terms' | 'privacy'; version: string },
): Promise<void> {
