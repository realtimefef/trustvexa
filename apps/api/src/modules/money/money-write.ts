...
export async function acquireMoneyClient(): Promise<MoneyTxClient> {
  return (await getClient()) as unknown as MoneyTxClient;
}
 ...
 */
export async function lockDealVersion(
  client: MoneyTxClient,
  dealId: string,
  expectedVersion: number,
): Promise<number> {
  const res = await client.query(
    `UPDATE deals SET version_no = version_no + 1, updated_at = now()
      WHERE id = $1 AND version_no = $2`,
    [dealId, expectedVersion],
  );
  if ((res.rowCount ?? 0) === 0) {
    throw new AppError(
      'concurrent_update',
 ...
 */
export async function runMoneyWrite<T>(req: MoneyWriteRequest<T>): Promise<MoneyWriteOutcome<T>> {
  ...
  try {
    ...
    const claim = await client.query<{ id: string }>(
      `INSERT INTO idempotency_keys
       ...
       RETURNING id`,
      [req.idempotencyKey, req.userId, req.actionType, req.dealId ?? null, hash, expiresAt],
    );

    if (claim.rows.length === 0) {
      // Key already committed by a prior request: replay (or reject mismatch).
      const existing = await client.query<IdempotencyRow>(
        `SELECT request_hash, status, response_ref FROM idempotency_keys WHERE key = $1 LIMIT 1`,
        [req.idempotencyKey],
      );
      ...
      return { replayed: true, result: JSON.parse(row.response_ref) as T };
    }

    // We own the key: perform the work and record the result, all-or-nothing.
    const result = await req.work(client);
    await client.query(
      `UPDATE idempotency_keys SET status = 'succeeded', response_ref = $2 WHERE key = $1`,
      [req.idempotencyKey, JSON.stringify(result ?? null)],
    );
