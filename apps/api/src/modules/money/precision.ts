 ...
 */
export const DEFAULT_PRECISION_RULES: readonly PrecisionRule[] = [
  {
    coin: 'BTC',
    network: 'bitcoin',
    decimals: 8,
...
export function precisionKey(coin: string, network: string): string {
  return `${coin.toUpperCase()}:${network.toLowerCase()}`;
}

const DEFAULT_RULE_INDEX: ReadonlyMap<string, PrecisionRule> = new Map(
  DEFAULT_PRECISION_RULES.map((rule) => [precisionKey(rule.coin, rule.network), rule]),
);
 ...
 */
export function getPrecisionRule(
  coin: string,
  network: string,
  rules?: readonly PrecisionRule[],
): PrecisionRule {
  if (rules) {
    const key = precisionKey(coin, network);
    for (const rule of rules) {
      if (precisionKey(rule.coin, rule.network) === key) return rule;
    }
    throw new Error(`No money_precision_rules entry for ${key}`);
  }
  const found = DEFAULT_RULE_INDEX.get(precisionKey(coin, network));
  if (!found) throw new Error(`No money_precision_rules entry for ${precisionKey(coin, network)}`);
  return found;
}
...
export function toSmallestUnit(amount: string, decimals: number): bigint {
  ...
  if (fracPart.length > decimals) {
    throw new Error(
      `Amount ${JSON.stringify(amount)} has more fractional digits than the coin's ${decimals} decimals`,
    );
  }
 ...
 * `toSmallestUnit`.
 */
export function fromSmallestUnit(units: bigint, decimals: number): string {
  assertValidDecimals(decimals);
  const negative = units < 0n;
  const magnitude = negative ? -units : units;
  const sign = negative ? '-' : '';
  if (decimals === 0) return `${sign}${magnitude.toString()}`;
...
export function meetsMinTransfer(units: bigint, rule: PrecisionRule): boolean {
  if (rule.minTransferSmallestUnit === undefined) return true;
