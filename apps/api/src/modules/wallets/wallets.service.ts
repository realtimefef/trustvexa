...
import { validateWalletAddress, type ValidationOutcome } from './address-validation.js';
...
import { runPreflight, WITHDRAWAL_PREFLIGHT_ORDER, type PreflightContext } from '../money/payout-preflight.js';
import { runMoneyWrite } from '../money/money-write.js';
import {
  loadAllowlistStatus,
  loadOperatorCap,
  type PayoutQueueTxClient,
} from '../money/payout-queue.repository.js';
...
export async function getAddressBook(userId: string): Promise<AddressBookEntry[]> {
...
export async function addAddress(
  ...
  const outcome = validateWalletAddress(
    input.coin as SupportedCoin,
    input.network as SupportedNetwork,
    input.address,
  );
...
export async function getChangeRequests(userId: string): Promise<ChangeRequestEntry[]> {
...
async function fetchOnChainBalance(
  ...
  if (!rpcUrl) {
    console.warn(
      `[wallets] RPC not configured: env var ${envKey} is missing. ` +
        `Balance for ${coin} on ${network} is unavailable.`,
    );
    throw new AppError(
      'rpc_unavailable',
      `On-chain balance query is temporarily unavailable because the network RPC is not configured.`,
      503,
    );
  }
...
export async function getWalletInfo(userId: string): Promise<WalletInfo> {
  ...
  const payoutAddrsRes = await query<{ coin: string; network: string; address_enc: string }>(
    `SELECT coin, network, address_enc FROM user_payout_addresses WHERE user_id = $1`,
    [userId],
  );

  const addresses = await Promise.all(
    payoutAddrsRes.rows.map(async (row) => ({
      network: row.network,
  ...
  const queryAddresses = await Promise.all(
    payoutAddrsRes.rows.map(async (r) => ({
      coin: r.coin,
      network: r.network,
  ...
  for (const item of queryAddresses) {
    ...
    if (!balanceUnavailable) {
      try {
        ...
        const rateRes = await dbQuery<{ rate: string }>(
          `SELECT rate::text FROM fx_price_snapshots
    ...
    const bal: WalletInfo['balances'][number] = {
      coin: item.coin,
      network:
        item.network === 'ETH' ? 'Ethereum' : item.network === 'SOLANA' ? 'Solana' : item.network,
...
export async function withdraw(
  ...
  if (process.env.MAINNET_ENABLED !== 'true') {
    throw new AppError(
      'mainnet_not_enabled',
      'Withdrawals are disabled until the operator completes the go-live checklist.',
      503,
    );
  }
  ...
  if (precNetwork === 'ethereum') precNetwork = 'ethereum';
  else if (precNetwork === 'tron') precNetwork = 'tron';
  else if (precNetwork === 'solana') precNetwork = 'solana';
  else if (precNetwork === 'bnb chain') precNetwork = 'bsc';
  ...
  let capturedPayoutId: string | undefined;
  let capturedVersionNo: number | undefined;

  const { result } = await runMoneyWrite<WithdrawResult>({
    idempotencyKey,
    ...
    work: async (client) => {
      ...
      const txClient = client as unknown as PayoutQueueTxClient;
      ...
      if (input.address) {
        ...
        const holdRows = await client.query<{ new_address_enc: string }>(
          `SELECT new_address_enc FROM wallet_change_requests
             WHERE user_id = $1 AND status = 'pending' AND hold_until > now()`,
          [userId],
        );
      ...
      if (preflight.failedCheck !== null) {
        throw new AppError(
          'preflight_failed',
          `Withdrawal preflight check failed: ${preflight.failedCheck}`,
          422,
        );
      }

      // Atomically insert the payout row inside the idempotency transaction.
      const insertRes = await client.query<{ id: string; version_no: number }>(
