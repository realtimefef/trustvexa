...
function readRawBody(req: Request, maxBytes: number = 50 * 1024 * 1024): Promise<Buffer> {
...
export async function viewFile(req: Request, res: Response): Promise<void> {
  ...
  const dbRes = await query<AttachmentDetails>(
    `SELECT a.id, a.kind, a.file_key, a.mime_type, a.scan_status, c.deal_id
       FROM message_attachments a
       JOIN messages m ON m.id = a.message_id
       JOIN chats c ON c.id = m.chat_id
       JOIN deals d ON d.id = c.deal_id
      WHERE a.id = $1
        AND (d.buyer_id = $2 OR d.seller_id = $2 OR d.middleman_id = $2)`,
    [fileId, userId],
  );
  ...
  if (!attachment) {
    ...
    const evidenceRes = await query<AttachmentDetails>(
      `SELECT e.id, 'evidence' AS kind, e.file_key, e.mime_type, 'clean' AS scan_status, dp.deal_id
...
export async function serveFile(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (typeResult) contentType = typeResult.mime;
  } catch {
    // fallback
  }
