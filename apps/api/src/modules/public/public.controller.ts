...
export async function getPolicyVersions(_req: Request, res: Response): Promise<void> {
...
export async function streamAvatar(req: Request, res: Response): Promise<void> {
  ...
  const dbRes = await query<{ username: string; avatar_file_key: string | null }>(
    `SELECT username, avatar_file_key FROM users WHERE id = $1`,
    [userId],
  );
  ...
  if (user.avatar_file_key) {
    ...
    try {
      ...
      if (bytes) {
        ...
        if (user.avatar_file_key.endsWith('.png')) contentType = 'image/png';
        else if (user.avatar_file_key.endsWith('.gif')) contentType = 'image/gif';
        else if (user.avatar_file_key.endsWith('.webp')) contentType = 'image/webp';
        else if (user.avatar_file_key.endsWith('.svg')) contentType = 'image/svg+xml';
...
export async function getServiceHours(_req: Request, res: Response): Promise<void> {
  const dbRes = await query<{
    day_of_week: number;
    open_time: string;
    close_time: string;
    timezone: string;
  }>(`SELECT day_of_week, open_time, close_time, timezone FROM service_hours ORDER BY day_of_week`);
  ...
  const holidaysRes = await query<{ holiday_date: string; label: string }>(
    `SELECT holiday_date, label FROM service_holidays ORDER BY holiday_date`,
  );
