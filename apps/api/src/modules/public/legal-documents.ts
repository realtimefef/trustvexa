...
export function findLegalDoc(docType: DocType): LegalDoc {
  const doc = LEGAL_DOCS.find((d) => d.docType === docType);
  if (!doc) throw new Error(`unknown legal doc type: ${docType}`);
  return doc;
}

/** Doc types that a user must (re)accept; feeds the signup + re-acceptance gate. */
export function acceptanceRequiredDocTypes(): DocType[] {
