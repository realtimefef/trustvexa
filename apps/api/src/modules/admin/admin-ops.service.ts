...
import { appendAdminAction, type TxClient } from './enforcement.repository.js';
...
export async function searchDeals(filters: {
  ...
  q?: string;
  limit?: number;
}): Promise<{ deals: DealSearchItem[] }> {
  const repoFilters: repo.DealSearchFilters = { limit: filters.limit ?? 50 };
  if (filters.status !== undefined) repoFilters.status = filters.status;
  if (filters.risk !== undefined) repoFilters.minRisk = filters.risk;
...
export async function searchUsers(filters: {
  ...
  if (filters.q !== undefined) repoFilters.q = filters.q;
  if (filters.status !== undefined) repoFilters.status = filters.status;
  if (filters.label !== undefined) repoFilters.label = filters.label;
...
function toHoldView(row: repo.HoldRow): HoldView {
  return {
    id: row.id,
    dealId: row.deal_id,
...
export async function listHolds(): Promise<{ holds: HoldView[] }> {
...
export async function placeHold(input: {
  ...
  visibleMessage: string | null;
  requestId: string;
}): Promise<{ hold: HoldView; alreadyOnHold: boolean }> {
...
export async function releaseHold(input: {
  ...
  reason: string;
  requestId: string;
}): Promise<{ hold: HoldView; alreadyReleased: boolean }> {
  return inTransaction(async (tx) => {
...
function toOverrideView(row: repo.OverrideRow): OverrideView {
  return {
    id: row.id,
...
export async function listOverrides(dealId?: string): Promise<{ overrides: OverrideView[] }> {
...
export async function recordOverride(input: {
  ...
  confirmed: boolean;
  requestId: string;
}): Promise<{ override: OverrideView; alreadyRecorded: boolean }> {
  if (!input.confirmed) {
...
function toNoteView(row: repo.NoteRow): NoteView {
  return {
    id: row.id,
...
export async function addNote(input: {
  ...
  targetId: string;
  note: string;
}): Promise<{ note: NoteView; alreadyExists: boolean }> {
  return inTransaction(async (tx) => {
...
function toPauseView(row: repo.PauseRow): PauseView {
  return {
    id: row.id,
    scope: row.scope,
    reason: row.reason,
...
export async function listPauses(): Promise<{ pauses: PauseView[] }> {
...
export async function startPause(input: {
  ...
  reason: string;
  requestId: string;
}): Promise<{ pause: PauseView; alreadyActive: boolean }> {
  return inTransaction(async (tx) => {
...
export async function endPause(input: {
  ...
  reason: string;
  requestId: string;
}): Promise<{ pause: PauseView; alreadyEnded: boolean }> {
  return inTransaction(async (tx) => {
...
function toFlagView(row: repo.FeatureFlagRow): FeatureFlagView {
  return {
...
export async function listFeatureFlags(): Promise<{ flags: FeatureFlagView[] }> {
...
export async function getAuditLog(): Promise<{ auditLog: AuditLogItem[] }> {
...
export async function createAnnouncement(input: {
  ...
  endsAt?: string | null;
  requestId: string;
}): Promise<{ announcement: AnnouncementView }> {
  return inTransaction(async (tx) => {
...
export async function searchChats(limit?: number): Promise<{ chats: ChatSearchItem[] }> {
