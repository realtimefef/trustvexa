...
import {
  ...
  markRolledBack,
  requestChange,
  type AdminSettingChangeRow,
  type TxClient,
} from '../dashboard/admin-setting.repository.js';
...
function toView(row: AdminSettingChangeRow): SettingChangeView {
  return {
    id: row.id,
...
function cooldownElapsed(cooldownUntil: string | null): boolean {
...
export async function requestSettingChange(
  actorId: string,
  input: SettingChangeRequestInput,
...
export async function applySettingChange(changeId: string): Promise<SettingChangeView> {
  const client = (await getClient()) as unknown as PooledTx;
  try {
    await client.query('BEGIN');
    ...
    if (!canApplyChange(existing.status as SettingChangeStatus, elapsed)) {
      const message =
        existing.status === 'cooling_down' && !elapsed
    ...
    if (!applied) {
      throw new AppError(
        'apply_conflict',
        'The setting change could not be applied because its state changed.',
        409,
      );
    }
...
export async function rollbackSettingChange(changeId: string): Promise<SettingChangeView> {
  const client = (await getClient()) as unknown as PooledTx;
  try {
    await client.query('BEGIN');
    const existing = await getChangeById(client, changeId);
    ...
    if (!canRollback(existing.status as SettingChangeStatus)) {
    ...
    if (!rolled) {
      throw new AppError(
        'rollback_conflict',
        'The setting change could not be rolled back because its state changed.',
        409,
      );
    }
    const updated = await getChangeById(client, changeId);
...
export async function listSettingChanges(settingKey: string): Promise<SettingChangeHistory> {
  const client = (await getClient()) as unknown as PooledTx;
  try {
