...
import type { DealRole, TermsDealRow } from './terms.repository.js';
...
function roleForUser(deal: TermsDealRow, userId: string): DealRole {
...
function toAgreementSource(deal: TermsDealRow): DealAgreementSource {
  return {
    id: deal.id,
...
export async function getAgreement(args: { dealId: string; requesterId: string }) {
...
export async function acceptTerms(args: {
  ...
  if (deal.status !== 'Verified') {
    throw new AppError(
      ...
      `Terms can only be accepted while the deal is Verified (currently ${deal.status}).`,
      409,
    );
  }
  ...
  if (currentTerms !== null && args.input.acceptedTermsVersion !== currentTerms) {
    throw new AppError(
      'stale_terms_version',
      `Terms version ${args.input.acceptedTermsVersion} is out of date; re-accept ${currentTerms}.`,
      409,
    );
  }
  if (currentDispute !== null && args.input.acceptedDisputePolicyVersion !== currentDispute) {
    throw new AppError(
      'stale_dispute_policy_version',
      `Dispute policy version ${args.input.acceptedDisputePolicyVersion} is out of date; re-accept ${currentDispute}.`,
      409,
    );
  }
  ...
  let agreementDocumentNumber: string | null = null;
  if (bothAccepted) {
