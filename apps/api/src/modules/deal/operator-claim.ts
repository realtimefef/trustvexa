...
export async function isOperatorAccount(userId: string): Promise<boolean> {
 ...
 */
export async function claimDealForOperator(userId: string, dealId: string): Promise<void> {
  if (!(await isOperatorAccount(userId))) return;
  await query(
    `UPDATE deals SET middleman_id = $1, updated_at = now()
      WHERE id = $2 AND middleman_id IS NULL`,
    [userId, dealId],
  );
  ...
  await query(
    `UPDATE connections SET middleman_id = $1, updated_at = now()
      WHERE deal_id = $2 AND middleman_id IS NULL`,
    [userId, dealId],
  );
}
