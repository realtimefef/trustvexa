...
export async function insertRiskFlag(client: TxClient, flag: RiskFlagInput): Promise<void> {
  await client.query(
    `INSERT INTO risk_flags (deal_id, user_id, flag_type, severity, details)
     VALUES ($1, $2, $3, $4, $5)`,
    [flag.dealId, flag.userId, flag.flagType, flag.severity, flag.details],
  );
}
 ...
 */
export async function logRiskFlag(flag: RiskFlagInput): Promise<void> {
  await query(
    `INSERT INTO risk_flags (deal_id, user_id, flag_type, severity, details)
     VALUES ($1, $2, $3, $4, $5)`,
    [flag.dealId, flag.userId, flag.flagType, flag.severity, flag.details],
 ...
 */
export async function applyMiddlemanReviewHold(
  client: TxClient,
  dealId: string,
  riskScore: number,
): Promise<void> {
  await client.query(
    `UPDATE deals
      ...
      WHERE id = $1`,
    [dealId, MIDDLEMAN_REVIEW_HOLD, riskScore],
  );
}
