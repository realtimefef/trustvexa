...
import {
  isAmendmentFullyApproved,
  isCancellationMutuallyApproved,
  isFundedActive,
  requiresMiddlemanApproval,
} from './amendment.js';
import * as amendRepo from './amendment.repository.js';
import type { AmendmentDealRow } from './amendment.repository.js';
import type {
  DecisionInput,
...
function roleForUser(deal: AmendmentDealRow, userId: string): DealRole {
...
function partyRoleForUser(deal: AmendmentDealRow, userId: string): 'buyer' | 'seller' {
  const role = roleForUser(deal, userId);
  if (role === 'middleman') {
    throw new AppError(
      'middleman_cannot_request',
      'The middleman cannot originate a change request or cancellation.',
      403,
    );
  }
  return role;
...
async function loadDealOr404(dealId: string): Promise<AmendmentDealRow> {
...
export async function requestAmendment(args: {
  ...
  if (deal.status !== 'Confirmed') {
    throw new AppError(
      ...
      `Amendments can be requested only on a Confirmed deal (currently ${deal.status}).`,
      409,
    );
  }
...
export async function decideAmendment(args: {
  ...
  try {
    ...
    if (amendment.status !== 'pending') {
      throw new AppError(
        'amendment_not_actionable',
        `Amendment is '${amendment.status ?? 'unknown'}' and can no longer be decided.`,
        409,
      );
    }
...
export async function listAmendments(args: { dealId: string; userId: string }) {
  const deal = await loadDealOr404(args.dealId);
...
export async function requestCancellation(args: {
  ...
  if (isFundedActive(deal.status)) {
    ...
    try {
      ...
      return {
        cancellationId: id,
        status: 'routed_to_dispute' as const,
  ...
  if (deal.status !== 'Confirmed') {
    throw new AppError(
      ...
      `Mutual cancellation applies only to a Confirmed (pre-funding) deal (currently ${deal.status}).`,
      409,
    );
  }
  ...
  try {
    ...
    await client.query('COMMIT');
    return { cancellationId: id, status: 'pending' as const, requesterRole: role };
  } catch (err) {
    await client.query('ROLLBACK').catch(() => undefined);
    throw err;
  } finally {
...
export async function decideCancellation(args: {
  ...
  if (role === 'middleman') {
    throw new AppError(
      'use_middleman_decision',
      'The middleman must use the middleman-decision endpoint for escalated cancellations.',
      403,
    );
  }
  ...
  try {
    await client.query('BEGIN');
    const cancellation = await amendRepo.getCancellationForUpdate(
      client,
      args.dealId,
      args.cancellationId,
    );
    ...
    if (cancellation.status !== 'pending') {
      throw new AppError(
        'cancellation_not_actionable',
        `Cancellation is '${cancellation.status ?? 'unknown'}' and can no longer be decided.`,
        409,
      );
    }
    ...
    const updated = await amendRepo.getCancellationForUpdate(
      client,
      args.dealId,
      args.cancellationId,
    );
...
export async function decideCancellationAsMiddleman(args: {
  ...
  if (role !== 'middleman') {
    throw new AppError(
      'not_middleman',
      'Only the assigned middleman can decide an escalated cancellation.',
      403,
    );
  }
  ...
  try {
    ...
    if (cancellation.status !== 'escalated') {
      throw new AppError(
        'cancellation_not_escalated',
        `Only an escalated cancellation can be decided by the middleman (currently '${cancellation.status ?? 'unknown'}').`,
        409,
      );
    }
    resultStatus = args.input.decision === 'approve' ? 'cancelled' : 'rejected';
    becameCancelled = args.input.decision === 'approve';
    await amendRepo.setCancellationMiddlemanDecision(
      client,
      args.cancellationId,
      decisionText,
      resultStatus,
...
export async function listCancellations(args: { dealId: string; userId: string }) {
  const deal = await loadDealOr404(args.dealId);
  roleForUser(deal, args.userId);
