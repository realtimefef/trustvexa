...
const ACTION_TO_SCOPE: Readonly<Record<ProtectedAction, PauseScope>> = {
...
export function activePauses(pauses: readonly IncidentPause[]): IncidentPause[] {
...
export function isActionBlocked(
  ...
  for (const pause of active) {
    if (pause.scope === scope) return true;
...
export function pauseBanner(pauses: readonly IncidentPause[]): string | null {
  const active = activePauses(pauses);
