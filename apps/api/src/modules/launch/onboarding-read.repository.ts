...
export async function completedTaskKeys(userId: string): Promise<OnboardingTaskKey[]> {
  const res = await query<{ task_key: OnboardingTaskKey }>(
...
export async function markTaskComplete(userId: string, taskKey: OnboardingTaskKey): Promise<void> {
  await query(
