...
import { lockDealVersion, runMoneyWrite, type MoneyTxClient } from '../money/money-write.js';
import {
  runPreflight,
  type PreflightCheckName,
  type PreflightCheckResult,
  type PreflightContext,
} from '../money/payout-preflight.js';
import {
  loadAllowlistStatus,
  loadOperatorCap,
  recordPreflightChecks,
  updatePayoutStatus,
} from '../money/payout-queue.repository.js';
import {
  hasOpenDispute,
  isDealLedgerBalanced,
  isTokenContractAllowlisted,
  listPriorApprovers,
  lockDealForPayout,
  lockPayout,
  setPayoutPreflightStatus,
  type DealPayoutRow,
  type PayoutLockRow,
} from './payouts-write.repository.js';

/** Deal statuses from which a seller payout may be released. */
const PAYABLE_DEAL_STATUSES: ReadonlySet<string> = new Set(['Approved', 'PayoutQueued']);

/** USDT is the only token coin on the platform; the rest are native. */
function coinIsToken(coin: string): boolean {
...
function assertAssignedMiddleman(deal: DealPayoutRow, middlemanId: string): void {
...
async function buildPreflightContext(
  ...
  if (payout.payee_id && payout.address) {
    ...
    const { rows } = await client.query<{ new_address_enc: string; hold_until: Date }>(
      `SELECT new_address_enc, hold_until FROM wallet_change_requests
  ...
  return {
    ...
    chainSupported: (SUPPORTED_NETWORKS as readonly string[]).includes(payout.network),
...
function toView(result: ReturnType<typeof runPreflight>): PreflightView {
  return {
 ...
 */
export async function approvePayout(input: ApprovePayoutInput): Promise<ApprovePayoutResult> {
  const { result } = await runMoneyWrite<ApprovePayoutResult>({
    ...
    work: async (client) => {
      ...
      if (payout === null) throw notFound('Payout was not found.');
      const deal = await lockDealForPayout(client, payout.deal_id);
      if (deal === null) throw notFound('Payout was not found.');
      ...
      if (payout.status !== 'pending') {
        throw new AppError(
          'payout_not_pending',
          `A payout in status '${payout.status}' cannot be approved.`,
          409,
        );
      }
      ...
      const blocking =
        preflight.failedCheck !== null && preflight.failedCheck !== 'two_step_signing';
      if (blocking) {
      ...
      const newVersion = await updatePayoutStatus(
        client,
        payout.id,
        payout.version_no,
        'approved',
      ...
      if (newVersion === null) {
        throw new AppError(
          'concurrent_update',
          'The payout was modified concurrently; reload and retry.',
          409,
        );
      }
 ...
 */
export async function broadcastPayout(input: BroadcastPayoutInput): Promise<BroadcastPayoutResult> {
  if (process.env.MAINNET_ENABLED !== 'true') {
    throw new AppError(
      'mainnet_not_enabled',
      'Payout broadcasting is disabled until the operator completes the go-live checklist.',
      503,
    );
  }

  const { result } = await runMoneyWrite<BroadcastPayoutResult>({
    idempotencyKey: input.idempotencyKey,
    ...
    work: async (client) => {
      ...
      if (payout.status !== 'approved') {
        throw new AppError(
          'payout_not_approved',
          `A payout in status '${payout.status}' cannot be broadcast; it must be approved first.`,
          409,
        );
      }
