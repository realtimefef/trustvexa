 ...
 */
export async function checkFailedLoginAbuse(ip, _email) {
    try {
        ...
        if (count > 10) {
            ...
            if (!alreadyFlagged) {
                ...
                await query(`INSERT INTO abuse_flags (subject, type, details)
           VALUES ($1, $2, $3)`, [
                    `ip:${ip}`,
                    'failed_login',
 ...
 * If so, flag as abuse.
 */
export async function checkInviteCodeAbuse(userId, token) {
    try {
        const redis = getRedis();
        ...
        if (count > 5) {
            ...
            if (!alreadyFlagged) {
                ...
                await query(`INSERT INTO abuse_flags (subject, type, details)
           VALUES ($1, $2, $3)`, [`user:${userId}`, 'invalid_invite', `More than 5 attempts on invite token: ${token}`]);
            }
        }
    }
    catch (err) {
 ...
 * If so, flag as abuse.
 */
export async function checkDealCreationVelocity(userId) {
    try {
        const redis = getRedis();
        ...
        if (count > 20) {
            ...
            if (!alreadyFlagged) {
                ...
                await query(`INSERT INTO abuse_flags (subject, type, details)
           VALUES ($1, $2, $3)`, [`user:${userId}`, 'deal_velocity', `More than 20 deals created in 24 hours.`]);
            }
        }
    }
    catch (err) {
...
//# sourceMappingURL=abuse-detector.js.map