...
async function main() {
    ...
    if (!email || !password) {
        console.error('Usage: node dist/scripts/setup-middleman.js <email> <password> [username]\n' +
    ...
    const emailOwner = await query(`SELECT id, username FROM users WHERE email_hash = $1 LIMIT 1`, [emailHash]);
    if (emailOwner.rows[0] && emailOwner.rows[0].username !== username) {
        console.error(`That email already belongs to a different account ("${emailOwner.rows[0].username}"). ` +
    ...
    try {
        ...
        if (existing.rows[0]) {
            ...
            SET email_hash = $2,
                ...
                updated_at = now()
          WHERE id = $1`, [userId, emailHash, emailEnc, passwordHash]);
         ...
         RETURNING id`, [username, emailHash, emailEnc, passwordHash]);
        ...
        await client.query(`INSERT INTO trust_events (user_id, change, reason, deal_id)
...
//# sourceMappingURL=setup-middleman.js.map