 ...
 */
export async function listActiveForAudience(userId, audience) {
    const res = await query(`SELECT a.id, a.title, a.body, a.audience, a.starts_at, a.ends_at, a.created_at,
            ar.read_at
      ...
      LIMIT 100`, [userId, audience]);
    return res.rows;
}
/** Whether an announcement exists at all (visibility-independent). */
export async function announcementExists(id) {
...
export async function markAnnouncementRead(announcementId, userId) {
    const res = await query(`INSERT INTO announcement_reads (announcement_id, user_id, read_at)
     VALUES ($1, $2, now())
     ON CONFLICT (announcement_id, user_id)
     DO UPDATE SET read_at = COALESCE(announcement_reads.read_at, EXCLUDED.read_at)
     RETURNING read_at`, [announcementId, userId]);
    const value = res.rows[0]?.read_at ?? new Date();
    return value instanceof Date ? value.toISOString() : String(value);
}
export async function getAnnouncementForUser(announcementId, userId, audience) {
    const res = await query(`SELECT a.id, a.title, a.body, a.audience, a.starts_at, a.ends_at, a.created_at,
            ar.read_at
       FROM announcements a
       LEFT JOIN announcement_reads ar
      ...
      LIMIT 1`, [announcementId, userId, audience]);
