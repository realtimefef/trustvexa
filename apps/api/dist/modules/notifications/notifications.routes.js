...
import { listNotificationsQuerySchema, notificationIdParamSchema, pushSubscribeSchema, upsertPreferenceSchema, } from './notifications.schemas.js';
const ROLES = ['user', 'middleman'];
export function notificationsRouter() {
    const router = Router();
    router.get('/', ...apiChain({ schemas: { query: listNotificationsQuerySchema }, roles: ROLES }), asyncHandler(controller.listNotifications));
    router.post('/read-all', ...apiChain({ roles: ROLES, enforceIdempotency: true }), asyncHandler(controller.markAllRead));
    router.get('/preferences', ...apiChain({ roles: ROLES }), asyncHandler(controller.getPreferences));
    router.put('/preferences', ...apiChain({
        schemas: { body: upsertPreferenceSchema },
        roles: ROLES,
        enforceIdempotency: true,
    }), asyncHandler(controller.upsertPreference));
    router.post('/push/subscribe', ...apiChain({ schemas: { body: pushSubscribeSchema }, roles: ROLES, enforceIdempotency: true }), asyncHandler(controller.subscribePush));
    router.patch('/:id/read', ...apiChain({
