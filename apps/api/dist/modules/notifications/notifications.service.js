...
import { findNotificationOwner, listNotifications, listPreferences, } from './notifications-read.repository.js';
...
export async function listForUser(userId, params) {
...
export async function markAllRead(userId) {
    return withTransaction(async (client) => {
        const res = await client.query(`UPDATE notifications SET read_at = now() WHERE user_id = $1 AND read_at IS NULL`, [userId]);
 ...
 */
export async function upsertPreference(userId, input) {
    return withTransaction(async (client) => {
        const res = await client.query(`INSERT INTO notification_preferences (user_id, event_type, channel, enabled)
            VALUES ($1, $2, $3, $4)
       ON CONFLICT (user_id, event_type, channel)
       DO UPDATE SET enabled = EXCLUDED.enabled
         RETURNING event_type, channel, enabled`, [userId, input.eventType, input.channel, input.enabled]);
...
export async function subscribePush(userId, input) {
    return withTransaction(async (client) => {
        const existing = await client.query(`SELECT id FROM push_subscriptions
        WHERE user_id = $1 AND endpoint = $2 AND revoked_at IS NULL
        ORDER BY created_at DESC
        LIMIT 1`, [userId, input.endpoint]);
        const current = existing.rows[0];
        if (current) {
            await client.query(`UPDATE push_subscriptions
            SET p256dh_key = $1, auth_key = $2, device = $3
          WHERE id = $4`, [input.p256dhKey, input.authKey, input.device ?? null, current.id]);
            return { id: current.id, endpoint: input.endpoint };
        }
        const inserted = await client.query(`INSERT INTO push_subscriptions (user_id, endpoint, p256dh_key, auth_key, device)
            VALUES ($1, $2, $3, $4, $5)
         RETURNING id, endpoint`, [userId, input.endpoint, input.p256dhKey, input.authKey, input.device ?? null]);
