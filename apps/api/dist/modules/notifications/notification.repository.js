...
export async function insertNotification(client, input) {
    const res = await client.query(`INSERT INTO notifications (user_id, deal_id, type, payload, priority, pinned)
...
export async function markRead(client, notificationId, userId) {
    await client.query(`UPDATE notifications SET read_at = now() WHERE id = $1 AND user_id = $2 AND read_at IS NULL`, [notificationId, userId]);
}
/**
 * Resolve whether a channel is enabled for a user + event type. Defaults to
 * enabled when no explicit preference row exists.
 */
export async function isChannelEnabled(client, userId, eventType, channel) {
    const res = await client.query(`SELECT enabled FROM notification_preferences
		  WHERE user_id = $1 AND event_type = $2 AND channel = $3
		  LIMIT 1`, [userId, eventType, channel]);
    const row = res.rows[0];
