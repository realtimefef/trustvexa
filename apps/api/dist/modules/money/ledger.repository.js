 ...
 */
export async function findOrCreateAccount(client, key) {
    const owner = key.ownerUserId ?? null;
    const deal = key.dealId ?? null;
    const existing = await client.query(`SELECT id FROM ledger_accounts
        ...
        LIMIT 1`, [key.accountType, key.coin, key.network, owner, deal]);
    const found = existing.rows[0];
    if (found)
        return found.id;
    const inserted = await client.query(`INSERT INTO ledger_accounts (account_type, coin, network, owner_user_id, deal_id)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id`, [key.accountType, key.coin, key.network, owner, deal]);
 ...
 */
export async function postEntryGroup(client, group, refs = {}) {
...
export async function postEntryGroups(client, groups, refs = {}) {
