...
export function isStuckTx(submittedAtIso, nowIso, stuckAfterSeconds) {
 ...
 */
export function planStuckTxAction(chainSupportsRbf, canBumpFee) {
    if (chainSupportsRbf)
        return 'replace_by_fee';
    if (canBumpFee)
        return 'fee_bump';
 ...
 */
export function assertNotDuplicatePayingTx(original, replacement) {
 ...
 */
export function sweepPlan(hotBalanceSmallestUnit, sweepThresholdSmallestUnit, targetReserveSmallestUnit) {
...
export function gasReserveStatus(balanceSmallestUnit, minReserveSmallestUnit, topUpToSmallestUnit) {
    const low = balanceSmallestUnit < minReserveSmallestUnit;
    const topUp = low && topUpToSmallestUnit > balanceSmallestUnit
 ...
 */
export function isLookAlikeAddress(real, candidate, edge = 4) {
    if (real === candidate)
        return false;
    if (real.length < edge * 2 || candidate.length < edge * 2)
        return false;
...
function scan(value, hit) {
    if (typeof value === 'string') {
        if (KEY_PATTERNS.some((re) => re.test(value)))
            hit();
        return;
    }
...
export function assertNoRawKeyMaterial(record) {
    let found = false;
    scan(record, () => {
        found = true;
    });
    if (found)
        throw new RawKeyMaterialError('record contains raw key material; refusing to persist');
}
