 ...
 */
export const WITHDRAWAL_PREFLIGHT_ORDER = PREFLIGHT_ORDER.filter((c) => c !== 'two_step_signing');
    ...
    switch (check) {
        ...
        case 'token_contract_allowlisted':
            return {
                ...
                message: !ctx.isToken || ctx.tokenContractAllowlisted
                    ? 'token contract ok'
        ...
        case 'amount_matches_snapshot':
            return {
                check,
                passed: ctx.amountSmallestUnit > 0n && ctx.amountSmallestUnit === ctx.snapshotPayoutSmallestUnit,
                message: ctx.amountSmallestUnit === ctx.snapshotPayoutSmallestUnit
        ...
        case 'two_step_signing':
            return {
                ...
                message: distinct(ctx.approverIds) >= 2
 ...
 */
export function runPreflight(ctx, order = PREFLIGHT_ORDER) {
    const results = [];
...
//# sourceMappingURL=payout-preflight.js.map