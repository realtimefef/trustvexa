...
export const DEAL_MIN_USD_CENTS = 40000n;
export const DEAL_MAX_USD_CENTS = 5000000n;
/** Flat minimum platform fee: $30. *(Requirement 15.3)* */
export const PLATFORM_FEE_MIN_CENTS = 3000n;
 ...
 * *(Requirement 15.5)*
 */
export const DEFAULT_SPLIT_BUYER_BPS = 5000n;
/** Clamp an arbitrary split-share value into the valid [0, 10000] bps range. */
export function normalizeSplitBuyerBps(value) {
    if (value === undefined)
        return DEFAULT_SPLIT_BUYER_BPS;
    if (value < 0n)
        return 0n;
...
export const PLATFORM_FEE_TIERS = [
    { lowerInclusiveCents: 40000n, upperExclusiveCents: 150000n, bps: 500n }, // 5%
    { lowerInclusiveCents: 150000n, upperExclusiveCents: 300000n, bps: 350n }, // 3.5%
    { lowerInclusiveCents: 300000n, upperExclusiveCents: 500000n, bps: 300n }, // 3%
    { lowerInclusiveCents: 500000n, upperExclusiveCents: 750000n, bps: 250n }, // 2.5%
    { lowerInclusiveCents: 750000n, upperExclusiveCents: 1200000n, bps: 200n }, // 2%
    { lowerInclusiveCents: 1200000n, upperExclusiveCents: 2000000n, bps: 175n }, // 1.75%
    { lowerInclusiveCents: 2000000n, upperExclusiveCents: 5000001n, bps: 135n }, // 1.35% (incl. $50,000)
];
export function assertDealAmountInRange(dealAmountCents) {
    if (dealAmountCents < DEAL_MIN_USD_CENTS || dealAmountCents > DEAL_MAX_USD_CENTS) {
        throw new Error(`Deal amount ${dealAmountCents} cents is outside the supported $400–$50,000 range`);
    }
}
/** Multiply then divide with round-half-up, in exact integer (bigint) math. */
export function mulDivRoundHalfUp(amount, numerator, denominator) {
    if (denominator <= 0n)
...
export function selectPlatformFeeBps(dealAmountCents) {
 ...
 */
export function platformFeeCents(dealAmountCents) {
    const percentageFee = mulDivRoundHalfUp(dealAmountCents, selectPlatformFeeBps(dealAmountCents), BPS_DENOMINATOR);
...
export function settlementFeeCents(dealAmountCents) {
    assertDealAmountInRange(dealAmountCents);
 ...
 */
export function platformFeeShares(platformFee, feePayer, splitBuyerBps = DEFAULT_SPLIT_BUYER_BPS) {
    switch (feePayer) {
        case 'buyer':
...
export function computeFeeBreakdown(input) {
    ...
    if (input.gasFeeCents < 0n)
        throw new Error('gasFeeCents must be non-negative');
 ...
 */
export function buildLineItems(breakdown) {
    return [
 ...
 */
export function isPayoutViable(breakdown) {
 ...
 * (Re-audit FIX-6)
 */
export function assertPayoutViable(breakdown) {
    if (!isPayoutViable(breakdown)) {
        throw new Error(`Seller payout would be negative (${breakdown.sellerReceivesCents}): platform/settlement fees + gas exceed the deal amount`);
    }
}
