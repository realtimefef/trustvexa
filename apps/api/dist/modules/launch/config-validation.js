 ...
 */
export function validateConfig(env, forMainnet) {
    const missing = [];
    const missingRecommended = [];
    if (!forMainnet)
        return { ok: true, missing, missingRecommended };
    for (const spec of ENV_SPECS) {
        if (!spec.requiredForMainnet)
            continue;
...
export function assertRuntimeConfig(env = process.env) {
    if (env.NODE_ENV !== 'test') {
        ...
        try {
            ...
            if (kek.length !== 32) {
                console.error(`CRITICAL ERROR: TRUSTVEXA_MASTER_KEK must be a valid 32-byte base64 string. Got ${kek.length} bytes.`);
                process.exit(1);
            }
        }
        catch (err) {
    ...
    if (env.MAINNET_ENABLED !== 'true')
        return;
    const result = validateConfig(env, true);
    if (!result.ok) {
        throw new Error(`MAINNET_ENABLED requires critical configuration keys: ${result.missing.join(', ')}`);
    }
    if (result.missingRecommended.length > 0) {
        ...
        console.warn(`[go-live] Mainnet is ON but optional feature services are unconfigured (degraded): ` +
