...
import { dealIdParamSchema, refundWalletBodySchema, submitTxBodySchema, } from './payment.schemas.js';
export function paymentRouter() {
    const router = Router();
    // GET /deals/:id/escrow-address — per-deal deposit address (parties only).
    router.get('/:id/escrow-address', ...apiChain({ schemas: { params: dealIdParamSchema }, roles: ['user', 'middleman'] }), asyncHandler(controller.getEscrowAddress));
    ...
    router.post('/:id/payment/submit-tx', ...apiChain({
    ...
    router.get('/:id/payment/status', ...apiChain({ schemas: { params: dealIdParamSchema }, roles: ['user', 'middleman'] }), asyncHandler(controller.getPaymentStatus));
    // POST /deals/:id/refund-wallet — buyer sets/updates the refund wallet.
    // Money/state route: requires an Idempotency-Key.
    router.post('/:id/refund-wallet', ...apiChain({
    ...
    router.post('/:id/payout-wallet', ...apiChain({
        schemas: { params: dealIdParamSchema, body: refundWalletBodySchema },
        roles: ['user'],
        enforceIdempotency: true,
    ...
    // Money/state route: requires an Idempotency-Key.
    router.post('/:id/payment-checklist-confirm', ...apiChain({
