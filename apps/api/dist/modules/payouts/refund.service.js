...
import { refundAmountSmallestUnit } from '../money/payout-queue.js';
import { hasProcessedRefund, insertRefundStatusEvent, lockDealForPayout, markDealRefunded, } from './payouts-write.repository.js';
/** Deal statuses from which a refund may be processed. */
const REFUNDABLE_DEAL_STATUSES = new Set([
    'Funded',
    'Disputed',
    'PayoutQueued',
    'Approved',
...
export async function processRefundForDeal(input) {
    const { result } = await runMoneyWrite({
        ...
        work: async (client) => {
            ...
            if (deal.middleman_id !== input.middlemanId) {
                throw new AppError('forbidden', 'Only the assigned middleman can process this refund.', 403);
            }
            ...
            if (!REFUNDABLE_DEAL_STATUSES.has(deal.status)) {
                throw new AppError('deal_not_refundable', `A deal in status '${deal.status}' cannot be refunded.`, 409);
            }
            ...
            if (refundAmount > 0n) {
                const group = buyerRefundPostings({ coin: deal.coin, network: deal.network, dealId: deal.id }, refundAmount);
            ...
            await client.query(`UPDATE payout_queue
            SET status = 'cancelled', updated_at = now()
          WHERE deal_id = $1
            AND status IN ('pending', 'approved')`, [deal.id]);
            return {
                dealId: deal.id,
                status: 'Refunded',
                cause: input.cause,
