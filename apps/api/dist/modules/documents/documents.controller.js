...
export async function getDealDocuments(req, res) {
...
export async function getAgreementPdf(req, res) {
    const userId = req.auth?.userId;
    if (!userId) {
        throw new Error('Authenticated user id missing after auth middleware.');
    }
...
export async function getDisputeDecisionPdf(req, res) {
    const userId = req.auth?.userId;
    if (!userId) {
        throw new Error('Authenticated user id missing after auth middleware.');
    }
...
export async function getDataExport(req, res) {
    const userId = req.auth?.userId;
    if (!userId) {
        throw new Error('Authenticated user id missing after auth middleware.');
    }
