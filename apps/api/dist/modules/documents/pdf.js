...
export function renderPdf(doc) {
    const ops = [];
    const text = (x, y, size, font, color, value) => {
        ops.push(`${color} rg`);
        ops.push('BT');
    ...
    for (const raw of doc.lines) {
        if (y < MARGIN + 40)
            break; // leave room for the footer; single page only
        const line = raw ?? '';
        if (line.trim() === '') {
            y -= 9;
            continue;
    ...
    text(MARGIN, MARGIN + 10, 8.5, 'F1', GREY, doc.footer ?? 'Generated by TrustVexa - trustvexa.com - This is a system-generated document.');
