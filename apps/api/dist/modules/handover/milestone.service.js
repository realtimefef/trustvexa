...
import { evaluateMilestoneRelease, pendingRequired, } from './milestones.js';
import { getMilestone, listDeliveryChecklist, lockDeal, markDeliveryChecklistItem, markMilestoneReleased, setDealStatusFrom, sumReleasedMilestones, } from './milestone.repository.js';
...
function checklistFromRows(rows) {
    ...
    const { result } = await runMoneyWrite({
        ...
        work: async (client) => {
            ...
            if (deal.middleman_id !== input.middlemanId) {
                throw new AppError('forbidden', 'Only the assigned middleman can release a milestone.', 403);
            }
            if (deal.status !== PAYOUT_QUEUED) {
                throw new AppError('not_payout_queued', 'A milestone can only be released while the deal is in the payout queue.', 409);
            }
            const milestone = await getMilestone(tx, input.dealId, input.milestoneId);
            if (milestone === null)
                throw notFound('Milestone was not found.');
            ...
            if (!decision.ok && decision.error) {
                if (decision.error === 'checklist_incomplete') {
                    ...
                    throw new AppError('checklist_incomplete', `The delivery checklist is not complete: ${pending.join(', ') || 'no items recorded'}.`, RELEASE_ERROR_STATUS.checklist_incomplete);
                }
                throw new AppError(decision.error, `Milestone release rejected: ${decision.error.replace(/_/gu, ' ')}.`, RELEASE_ERROR_STATUS[decision.error]);
            }
            ...
            if (marked === 0) {
                throw new AppError('milestone_release_conflict', 'The milestone state changed; reload and retry.', 409);
            }
            // Partial seller payout out of escrow for this milestone amount.
            if (amount > 0n) {
                await postEntryGroup(client, sellerPayoutPostings({ coin: deal.coin, network: deal.network, dealId: input.dealId }, amount), { dealId: input.dealId });
            }
    ...
    const { result } = await runMoneyWrite({
        ...
        work: async (client) => {
            ...
            if (deal === null)
                throw notFound('Deal was not found.');
            if (deal.middleman_id !== input.middlemanId) {
                throw new AppError('forbidden', 'Only the assigned middleman can update the delivery checklist.', 403);
            }
