...
import { accountActionSchema, changeEmailSchema, changePasswordSchema, confirmStepUpSchema, forgotPasswordSchema, resetPasswordSchema, setRecoveryEmailSchema, stepUpSchema, verifyEmailSchema, acceptPolicySchema, } from './auth.schemas.js';
import * as recovery from './recovery.controller.js';
import * as sessions from './sessions.controller.js';
const AUTHED = ['user', 'middleman'];
...
export function recoveryRouter() {
    ...
    // Authenticated.
    router.post('/request-email-verification', ...apiChain({ roles: AUTHED }), asyncHandler(recovery.requestEmailVerification));
    router.post('/change-password', ...apiChain({ roles: AUTHED, schemas: { body: changePasswordSchema } }), asyncHandler(recovery.changePassword));
    router.post('/change-email', ...apiChain({ roles: AUTHED, schemas: { body: changeEmailSchema } }), asyncHandler(recovery.changeEmail));
    router.post('/recovery-email', ...apiChain({ roles: AUTHED, schemas: { body: setRecoveryEmailSchema } }), asyncHandler(recovery.setRecoveryEmail));
    router.post('/step-up', ...apiChain({ roles: AUTHED, schemas: { body: stepUpSchema } }), asyncHandler(recovery.requestStepUp));
    router.post('/step-up/confirm', ...apiChain({ roles: AUTHED, schemas: { body: confirmStepUpSchema } }), asyncHandler(recovery.confirmStepUp));
    router.post('/accept-policy', ...apiChain({ roles: AUTHED, schemas: { body: acceptPolicySchema } }), asyncHandler(recovery.acceptPolicy));
    return router;
}
...
export function accountRouter() {
    const router = Router();
    router.post('/deactivate', ...apiChain({ roles: AUTHED, schemas: { body: accountActionSchema } }), asyncHandler(account.deactivate));
    router.post('/reactivate', ...apiChain({ roles: AUTHED }), asyncHandler(account.reactivate));
    router.post('/delete', ...apiChain({ roles: AUTHED, schemas: { body: accountActionSchema } }), asyncHandler(account.requestDeletion));
    return router;
}
//# sourceMappingURL=auth-extra.routes.js.map