...
import { addAddressSchema, addressIdParamSchema, changeRequestSchema, validateAddressSchema, withdrawSchema, } from './wallets.schemas.js';
export function walletsRouter() {
    const router = Router();
    router.get('/', ...apiChain({ roles: ['user', 'middleman'] }), asyncHandler(controller.getWalletInfo));
    router.post('/withdraw', ...apiChain({
