...
import { createDerivedKeyProvider, decryptField, encryptField, } from '@trustvexa/shared/crypto';
let cached = null;
let kekUnavailable = false;
/** Read the active version for a purpose, defaulting to 1 when none exists. */
async function activeVersionForPurpose(purpose) {
    try {
        const res = await query(`SELECT version FROM encryption_key_versions
         WHERE key_purpose = $1 AND status = 'active'
         ORDER BY version DESC LIMIT 1`, [purpose]);
 ...
 */
export async function sealPii(value, purpose = 'pii') {
 ...
 */
export async function openPii(token) {
    if (token === null || token === undefined)
        return null;
    if (!token.startsWith('tv1:'))
        return token;
    const provider = tryGetKeyProvider();
    if (provider === null)
        return token;
