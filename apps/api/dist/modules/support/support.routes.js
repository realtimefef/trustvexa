...
import { listAdminTickets, getAdminTicket, updateAdminTicketStatus, replyToTicket, VALID_TICKET_STATUSES, } from './support.service.js';
...
export function supportRouter() {
    const router = Router();
    // ── User routes ────────────────────────────────────────────────────────────
    router.post('/tickets', ...apiChain({ schemas: { body: createTicketSchema }, roles: ROLES, enforceIdempotency: true }), asyncHandler(controller.createTicket));
    router.get('/tickets', ...apiChain({ roles: ROLES }), asyncHandler(controller.listTickets));
    router.get('/tickets/:id', ...apiChain({ schemas: { params: ticketIdParamSchema }, roles: ROLES }), asyncHandler(controller.getTicket));
    // ── Middleman (admin) routes ────────────────────────────────────────────────
    router.get('/admin/tickets', ...apiChain({
    ...
    router.get('/admin/tickets/:id', ...apiChain({ schemas: { params: adminTicketParamSchema }, roles: ['middleman'] }), asyncHandler(async (req, res) => {
        const id = requireParam(req, 'id');
        const result = await getAdminTicket(id);
        res.status(200).json(result);
    }));
    router.post('/admin/tickets/:id/reply', ...apiChain({
    ...
    router.patch('/admin/tickets/:id/status', ...apiChain({
