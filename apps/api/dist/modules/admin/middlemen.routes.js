...
export async function listMiddlemen(_req, res) {
    const result = await query(`SELECT u.id, u.username,
...
export function middlemenRouter() {
    const router = Router();
    router.get('/', ...apiChain({ roles: ['user', 'middleman'] }), asyncHandler(listMiddlemen));
    return router;
}
//# sourceMappingURL=middlemen.routes.js.map