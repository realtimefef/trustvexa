...
export async function lockUser(tx, userId) {
    const { rows } = await tx.query(`SELECT id, account_status, account_label, trust_level
 ...
 */
export async function appendAdminAction(tx, input) {
    const prev = await tx.query(`SELECT entry_hash FROM admin_actions ORDER BY created_at DESC LIMIT 1`);
    ...
    const { rows } = await tx.query(`INSERT INTO admin_actions
    ...
    const row = rows[0];
    if (!row)
        throw new Error('appendAdminAction returned no row');
    return row.id;
}
/** Set the target user's account status (e.g. `blocked`, `under_review`, `active`). */
export async function setAccountStatus(tx, userId, status) {
...
export async function setAccountLabel(tx, userId, label) {
...
export async function insertUserBlock(tx, input) {
    await tx.query(`INSERT INTO user_blocks (blocker_id, blocked_user_id, block_type, reason)
     VALUES ($1, $2, 'middleman_platform_block', $3)`, [input.blockerId, input.blockedUserId, input.reason]);
}
/** Apply a trust-level delta and record the trust_events audit row. */
export async function applyTrustChange(tx, input) {
    const { rows } = await tx.query(`UPDATE users SET trust_level = trust_level + $2, updated_at = now()
       WHERE id = $1 RETURNING trust_level`, [input.userId, input.change]);
    ...
    const row = rows[0];
    if (!row)
        throw new Error('applyTrustChange returned no row');
...
export async function insertWarningNotice(tx, input) {
    await tx.query(`INSERT INTO user_warning_notices (user_id, warning_type, message)
     VALUES ($1, $2, $3)`, [input.userId, input.warningType, input.message]);
}
