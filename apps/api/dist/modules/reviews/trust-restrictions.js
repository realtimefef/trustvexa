...
function manualReviewBlock(count) {
    return {
 ...
 */
export function restrictionFor(input) {
...
export function nextConsequence(currentMissedCount) {
...
export function canCreateDeal(restriction, currentActiveDeals) {
    if (restriction.blockNewDeals)
        return false;
    if (restriction.maxActiveDeals !== null)
        return currentActiveDeals < restriction.maxActiveDeals;
    return true;
}
/** Apply the single controlled appeal decision recorded by the middleman (Req 26.10). */
export function applyAppeal(decision, cooldownHours) {
    switch (decision) {
        case 'reinstate':
...
//# sourceMappingURL=trust-restrictions.js.map