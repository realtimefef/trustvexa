...
export function reviewsRouter() {
    const router = Router();
    router.get('/users/:userId', ...apiChain({ schemas: { params: userIdParamSchema } }), asyncHandler(controller.getUserReviews));
    router.get('/me/trust', ...apiChain({ roles: ['user', 'middleman'] }), asyncHandler(controller.getMyTrust));
    // --- Middleman moderation (Plan §9, Requirement 25.4) ------------------
    // List every review about a user, including hidden ones, for triage.
    router.get('/moderation/users/:userId', ...apiChain({ schemas: { params: userIdParamSchema }, roles: ['middleman'] }), asyncHandler(controller.listReviewsForModeration));
    // Hide / unhide a review (audited, reason required, idempotent).
    router.post('/moderation/:reviewId', ...apiChain({
    ...
    router.get('/deals/:id/me', ...apiChain({
    ...
    router.get('/public', ...apiChain({ schemas: { query: publicReviewListQuerySchema } }), asyncHandler(controller.listPublicReviews));
