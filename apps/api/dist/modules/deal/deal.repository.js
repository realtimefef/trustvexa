...
export async function loadDealVersion(client, dealId) {
    const res = await client.query(`SELECT status, version_no FROM deals WHERE id = $1 LIMIT 1`, [dealId]);
 ...
 */
export async function loadLastEntryHash(client, dealId) {
    const res = await client.query(`SELECT entry_hash FROM escrow_logs WHERE deal_id = $1 ORDER BY created_at DESC LIMIT 1`, [dealId]);
 ...
 */
export async function applyDealStatus(client, dealId, toStatus, readVersion, nextVersion) {
    const res = await client.query(`UPDATE deals
        SET status = $1, version_no = $2, updated_at = now()
      WHERE id = $3 AND version_no = $4`, [toStatus, nextVersion, dealId, readVersion]);
 ...
 */
export async function insertEscrowLog(client, entry, action, visibility, 
