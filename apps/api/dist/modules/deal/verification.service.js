...
import { codesMatch, evaluateCode, generateVerificationCode, hashVerificationCode, } from './verification-code.js';
import { expireActiveCodes, getLatestCode, incrementAttempts, insertVerificationCode, loadDealParties, markVerified, } from './verification.repository.js';
export async function requestVerificationCode(args) {
    ...
    if (deal.status !== 'Agreed') {
        throw new AppError('verification_not_allowed', `Verification can only be requested while the deal is Agreed (currently ${deal.status}).`, 409);
    }
...
export async function submitVerificationCode(args) {
    ...
    if (row === null) {
        throw new AppError('no_active_code', 'No verification code has been requested for this deal.', 404);
    }
    // Reject used / expired / exhausted before spending an attempt.
    rejectUnusableCode(evaluateCode({
    ...
    if (attempts > cfg.maxVerificationAttempts) {
        throw new AppError('too_many_attempts', 'Too many verification attempts. Request a new code.', 429);
    }
...
function rejectUnusableCode(usability) {
    if (usability === 'ok') {
        return;
    }
    const map = {
