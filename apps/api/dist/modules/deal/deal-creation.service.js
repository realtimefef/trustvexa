...
import { assertPracticeNetwork, mainnetOperationAllowed, PracticeModeError, } from '../launch/practice.js';
import { DealAmountOutOfBoundsError, assertDealAmountWithinBounds } from './deal-amount.js';
import { insertDeal, insertDealTermsSnapshot, loadDealForDuplication, loadLatestTermsSnapshot, } from './deal-creation.repository.js';
import { applyMiddlemanReviewHold, insertRiskFlag, logRiskFlag, } from './deal-screening.repository.js';
...
import { insertEscrowAddress, } from '../money/escrow-address.repository.js';
import { ensureChat } from '../chat/chat.repository.js';
import { getConnectionById } from '../connections/connections.repository.js';
function toAppErrorOnBadAmount(err) {
    if (err instanceof DealAmountOutOfBoundsError) {
        throw new AppError('deal_amount_out_of_bounds', 'Deal amount must be between $400 and $50,000.', 422);
    }
    throw err;
}
...
export async function createDeal(args) {
    ...
    if (input.confirmLegal !== true) {
        throw new AppError('legal_confirmation_required', 'You must confirm the item is legal and allowed.', 422);
    }
    try {
    ...
    if (networkMode === 'mainnet' && !mainnetOperationAllowed()) {
        throw new AppError('mainnet_not_enabled', 'Mainnet deals are disabled until the operator completes the go-live checklist.', 503);
    }
    ...
    if (input.connectionId) {
        ...
        if (!conn.joiner_id) {
            throw new AppError('connection_not_joined', 'The other party has not joined this connection yet.', 409);
        }
        if (conn.deal_id) {
    ...
    if (screen.decision === 'block') {
        ...
        throw new AppError('prohibited_item', 'This item is prohibited and cannot be traded on TrustVexa.', 422);
    }
    const client = await acquireClient();
    try {
        await client.query('BEGIN');
        const policyCheck = await client.query(`SELECT pv.doc_type, pv.version AS active_version, ta.version AS accepted_version
       ...
       WHERE pv.doc_type IN ('terms', 'privacy')
        ...
        if (policyCheck.rows.some((r) => !r.accepted_version)) {
            throw new AppError('policy_re_acceptance_required', 'You must accept the latest Terms and Privacy Policy before creating a deal.', 403);
        }
        ...
        if (input.connectionId) {
            ...
            await client.query(`UPDATE connections
            SET deal_id = $2,
                creator_role = $3,
                updated_at = now()
          WHERE id = $1`, [input.connectionId, deal.id, creatorIsBuyer ? 'buyer' : 'seller']);
        }
        ...
        if (networkMode === 'mainnet') {
            ...
            if (!receiveAddress) {
                throw new AppError('escrow_address_unavailable', `No operator receive address is configured for ${input.coin} on ${input.network}. ` +
                    `Configure it in operator-receive-wallets before creating mainnet deals for this asset.`, 503);
            }
            await insertEscrowAddress(client, {
                dealId: deal.id,
                coin: input.coin,
                network: input.network,
        ...
        if (input.tags && input.tags.length > 0) {
           ...
           VALUES ($1, $2, $3)
           ON CONFLICT DO NOTHING`, [sellerId, deal.id, tag]);
            }
        }
 ...
 */
export async function duplicateDeal(args) {
    ...
    try {
        ...
        if (src === null) {
            throw new AppError('deal_not_found', 'Source deal was not found or is not owned by you.', 404);
        }
        ...
        const deal = await insertDeal(client, {
            ...
            feeSplitBuyerBps: (src.fee_payer ?? 'split') === 'split' ? (src.fee_split_buyer_bps ?? 5000) : null,
...
//# sourceMappingURL=deal-creation.service.js.map