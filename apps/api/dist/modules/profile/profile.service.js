...
export async function getMyProfile(userId) {
    const res = await query(`SELECT username, account_label, trust_level, account_status, created_at, avatar_file_key
       FROM users
      WHERE id = $1`, [userId]);
...
export async function getPreferences(userId) {
    const res = await query(`SELECT p.timezone, p.locale, p.theme, p.display_fiat, p.email_digest_frequency,
  ...
  LEFT JOIN notification_global_settings n ON n.user_id = p.user_id
    ...
    const userRes = await query(`SELECT recovery_email_enc FROM users WHERE id = $1`, [userId]);
    ...
    const row = res.rows[0];
    if (!row) {
        const nRes = await query(`SELECT quiet_hours_start, quiet_hours_end FROM notification_global_settings WHERE user_id = $1`, [userId]);
 ...
 */
export async function upsertPreferences(userId, input) {
    await query(`INSERT INTO user_preferences (user_id, timezone, locale, theme, display_fiat, email_digest_frequency,
    ...
    if ('quietHoursStart' in input || 'quietHoursEnd' in input) {
        ...
        const checkSettings = await query(`SELECT quiet_hours_start, quiet_hours_end FROM notification_global_settings WHERE user_id = $1`, [userId]);
        if (checkSettings.rows.length === 0) {
            await query(`INSERT INTO notification_global_settings (user_id, quiet_hours_start, quiet_hours_end)
         VALUES ($1, $2, $3)`, [userId, qStart ?? null, qEnd ?? null]);
        }
        else {
            ...
            await query(`UPDATE notification_global_settings
            SET quiet_hours_start = $2,
                quiet_hours_end = $3
          WHERE user_id = $1`, [userId, nextStart, nextEnd]);
        }
    }
...
export async function updateProfile(userId, input) {
    ...
    await query(`UPDATE users
        SET username = COALESCE($2, username),
            ...
            updated_at = now()
      WHERE id = $1`, [userId, input.username ?? null, input.avatarFileKey ?? null]);
    return getMyProfile(userId);
}
export async function updateAvatarUrl(userId, avatarFileKey) {
    await query(`UPDATE users
        SET avatar_file_key = $2,
            updated_at = now()
      WHERE id = $1`, [userId, avatarFileKey]);
}
