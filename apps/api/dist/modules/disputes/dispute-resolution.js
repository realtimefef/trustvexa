...
export function computeSettlement(req) {
    if (req.escrowAmount <= 0n)
        throw new SettlementValidationError('escrow_not_positive');
    switch (req.outcome) {
        case 'full_refund':
        ...
        case 'partial_split': {
            if (req.buyerShare === undefined)
                throw new SettlementValidationError('buyer_share_required');
...
export function settlementBalances(escrowAmount, split) {
    return (split.toBuyer >= 0n && split.toSeller >= 0n && split.toBuyer + split.toSeller === escrowAmount);
}
/** Map a resolved outcome to the deal-state-machine event that effects it. */
export function resolutionEvent(outcome) {
    switch (outcome) {
        case 'full_refund':
...
export function canResolveDispute(status, isMiddleman) {
...
export function isEvidenceLocked(evidence) {
    return evidence.lockedAt !== null;
}
export function canModifyEvidence(evidence) {
    return !isEvidenceLocked(evidence);
}
//# sourceMappingURL=dispute-resolution.js.map