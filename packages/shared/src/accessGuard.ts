...
function isOwner(actor: Actor, ownership: RowOwnership): boolean {
...
export async function assertRowAccess(
  ...
  mode: AccessMode,
  opts: { recorder?: PiiAccessRecorder; reason?: string } = {},
): Promise<void> {
  if (isOwner(actor, ownership)) return;
...
export function canAccessRow(actor: Actor, ownership: RowOwnership): boolean {
